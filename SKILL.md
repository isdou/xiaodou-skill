...
- **架构 B（流程拆解）**：
  - 顶部 `主标题` + `黑色胶囊金句框`。
  - 左侧输入端（想法/疑问） -> 中间控制台 UI（规则/代码块） -> 右侧输出端（灯泡小豆/自由门）。
...
#### 宫格单卡片内部布局规范 (单宫格自上而下)：
1. **顶栏标题 (Grid Title)**：醒目的黑体/粗体中文大标题（如 `原生家庭是第一段 System Prompt` 或 `你看不到的 System Prompt`）。
...
#### 🔴 画面细节约束：去除最底部黑色 Hashtag 标签栏
> [!IMPORTANT]
> [配图 1.jpg](file:///Users/suxiaohan/Desktop/小豆-skill/配图/1.jpg) 与 [配图 2.jpg](file:///Users/suxiaohan/Desktop/小豆-skill/配图/2.jpg) 的 4 宫格排版和插图表现**均属于标准合格的样式**。
> **唯一需要注意的控制细节**：在生成 4 宫格图片时，**只需去掉最底下那行黑色 Hashtag 标签栏**（即不生成如 `#豆豆 #底层逻辑...` 的黑底文字条），保持图片以 4 宫格主体展现。
...
### Step 3: Prompt 编写与生成
将 Blueprint 转换为精确 Prompt，包含小豆 IP 规范、2x2 四宫格布局指令、文字与图标排版指令，并强调 **Do not generate bottom hashtag bar**。
...
1. **[配图样例 1 (1.jpg)](file:///Users/suxiaohan/Desktop/小豆-skill/配图/1.jpg)** — 标准 2x2 四宫格布局，无最下黑条。
2. **[配图样例 2 (2.jpg)](file:///Users/suxiaohan/Desktop/小豆-skill/配图/2.jpg)** — 标准 2x2 四宫格布局（画面最底部的黑条需去除）。
